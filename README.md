...
| **出力の形を縛る v0.14.0（ノードごとに出力の形を選ぶ・リスト(n個)・配列を箇条書きに・日本語のノード名） | **いまここ** |
...
```
node tests/harness-runtime.mjs      # 自己テスト42件（Runtime回帰15 + Provider契約13 + HSL3 + 合流2 + Join/失敗の文2 + JSON強制と取得先4）
