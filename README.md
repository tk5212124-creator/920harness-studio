...
| **動き出す条件 v0.22.0（全部そろったら／どれか来たら／自分で組む（and・or・入れ子）・合流の連鎖を直す）** | **いまここ** |
...
```
node tests/harness-runtime.mjs      # 自己テスト59件（Runtime回帰 + Provider契約 + HSL + 合流 + ループ + 出口/取り出し + 条件/計算 + 動く順番）
