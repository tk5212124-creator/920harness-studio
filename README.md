...
| 端末が落ちないようにする v0.32.0（Checkpoint をまとめて書く＝28.7MB→4.1MB・保存できなかったら赤字で出す・文脈の窓に入らない文は送らない・空になる差し込みを診断で名指し・点検の行き止まりを error にしない） | 実装済み |
| 何十回呼んでも落ちない v0.33.0（webllm を1回ごとに resetChat＝KVの山 19281→1867 の90%減・needs_reset で engine を捨てて読み直す・実ハーネスを JSON 無改変で画面から実行する回帰テスト） | 実装済み |
| 落ちても記録が残る v0.34.0（ブラックボックス＝1手ごとに極小の記録を localStorage へ同期で置く。端末ごと落ちても「どのノードの何回目の推論で消えたか・送った文の字数・そのときの空きメモリ・端末の限界値」が残り、次に開いたときに知らせてそのままコピーできる。レンダラを本当にクラッシュさせる回帰テスト付き） | 実装済み |
| 実機の記録で分かった穴を塞ぐ v0.34.1（自己テストが記録を埋めていた＝300枠の282枠／実行ボタン以外の入口（1 Work Item・Retry・くらべる・モデルの帯）が記録されず「実行: (実行前)」になっていた／読み込みの記録に文脈の窓と必要メモリの目安を入れた／読み込み中に消えていたら知らせから「省メモリで試す（文脈1024）」を押せるようにした） | 実装済み |
| **何を入力して落ちたかを残す v0.35.0（入れた値・落ちた瞬間に送っていた文そのもの・生成の途中のトークン数・読み込みの途中の%を1件ずつ上書きで残す／推論ごとの tokens/秒／WebGPUが壊れたことを console.error から拾う（iPhoneでは他に見る場所が無い）／Safariではメモリ使用量が読めないことを記録に明記）** | **いまここ** |
...
### 9.1 落ちても残る記録（ブラックボックス）

端末ごと落ちると、画面のログもメモリの中の Run も消える。Checkpoint（Run 全体）は
落ちる瞬間には間に合わないことがある。そこで**1回の書き込みを極小にして localStorage に置く**
記録を別に持っている。localStorage は同期なので、`setItem` が返った分は落ちても消えない。

| キー | 中身 |
|---|---|
| `hbb.0` … `hbb.299` | 流れ（輪。300件を超えたら古いものから上書き） |
| `hbb.h` | 輪の先頭・件数・実行ID・版 |
| `hbb.cur` | いま通っている Work Item（1件を上書きし続ける） |
| `hbb.open` | **始めたのに終わっていない所**（モデルの読み込み・推論1回。終わったら消す） |
| `hbb.in` | その実行に入れた値（何を入力したか） |
| `hbb.last` | **落ちた瞬間に送っていた文**（system込み・長めに残す） |
| `hbb.gen` | 生成の途中（何トークンまで出たか・500msに1回） |
| `hbb.lprg` | 読み込みの途中（%と段階・%が動いたら書く） |

落ちると `hbb.open` が消えずに残る。これが**落ちた場所**そのものになる。
流れに載るのは `run`（実行開始）/ `dev`（UA・メモリ・JSヒープ上限・**WebGPU の限界値**）/
`load`（モデルの読み込み・モデル名・**文脈の窓（`ctx` が null は「指定なし＝モデルの既定」）**・
**必要メモリの目安**）/ `call`（推論1回・**送った文の字数**・maxTokens・時間・ヒープ）/
`vis`（表↔裏）/ `err`（拾えなかった例外）/ `exp`（くらべるの1回）/ `end` / `left`。

**実行の始まりで輪を空にする**（1回の実行に300枠を全部使う）。**自己テストは記録しない**
（開くだけで200件以上の実行を回すため。v0.34.0 の実機では300枠のうち282枠が自己テストで埋まっていた）。
記録する入口は 実行 / 1 Work Item / Retry / くらべる / モデルの帯からの手動読み込み、
加えて `invokeNode` の先に保険を置いてあるので、どの入口から engine に入っても
「実行: (実行前)」にはならない。

「途中で消えた」は、いちばん新しい `run` より後ろに `end` も `left` も無いか、`open` が残っている場合。
次に開いたときにいちばん上で知らせ、「落ちる前の記録」ボタンで全文を読み・コピー・保存できる
（「まとめてコピー」にも入る）。消えた場所が**モデルの読み込み**なら、その知らせに
「省メモリで試す（文脈1024）」も出る。**JSON の設定ではなく、いつも取っている。**

**メモリ使用量は出せない。** iPhone の Safari には読む口が無い（`performance.memory` は Chrome だけ、
`navigator.deviceMemory` も同じ）。タスクマネージャーのような数字の代わりに、**要求した量と進み方**を
残している: モデルの必要メモリの目安・文脈の窓・送った文の字数・**1秒あたりのトークン数**・読み込みの%・
置き場の使用量（ディスク）。「回を追って遅くなる」「同じ%で止まる」はここから読める。

**`console.error` も拾う。** WebGPU が壊れたこと（`device.lost` / uncaptured error）は WebLLM の中の
runtime が気づいているが、`console.error` に出すだけでアプリには渡ってこない。iPhone では console を
見る手段が無いので、`console.error` を通して記録に入れている（`console.warn` はメモリ・GPU に
関係する文だけ）。console そのものの動きは変えていない。

分かるのは「落ちる直前の状態」まで。**落ちた理由そのものは分からない**
（OS がタブを殺すときは何も残せない）。原因の断定はしていない。

`tests/harness-blackbox.mjs` は、実ハーネスの3回目の推論中に CDP の `Page.crash` で
**本当にレンダラを落として**、記録が残って読めることを確かめている。
...
```
node tests/harness-runtime.mjs      # 自己テスト89件（Runtime回帰 + Provider契約 + HSL + 合流 + ループ + 出口/取り出し + 条件/計算 + 動く順番 + エラーの扱い + 実行の記録 + 式で使える名前と演算子 + first_matchのpriority + wllamaの既定 + 先着 + 検証の適用範囲 + 記憶 + 読まないキーの診断 + 文脈の窓 + Checkpointのまとめ書き + engineのリセット + 落ちても残る記録 + 入力と送った文の記録）
