...
| **1件ずつ（Map）と集計 v0.27.0（配列の1件ずつ・入れ子の場・1件だけの失敗・続きから／Run比較の集計統計／多数決の「もう来ない票」）** | **いまここ** |
...
```
node tests/harness-runtime.mjs      # 自己テスト73件（Runtime回帰 + Provider契約 + HSL + 合流 + ループ + 出口/取り出し + 条件/計算 + 動く順番 + エラーの扱い + 実行の記録）
