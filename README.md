...
| **モジュールとくらべる v0.26.0（部分ハーネスの再利用／まとめる・多数決・点検ノード／Run比較／ノードごとの時刻）** | **いまここ** |
...
```
node tests/harness-runtime.mjs      # 自己テスト69件（Runtime回帰 + Provider契約 + HSL + 合流 + ループ + 出口/取り出し + 条件/計算 + 動く順番 + エラーの扱い + 実行の記録）
