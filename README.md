...
**AIに直させて、その出力をそのまま口に入れるとノードと線になる。** 今の構造＋文法＋指示を1つの文面にして渡し、
返ってきた文（``` 付きでも前置き付きでもよい）を貼ると、**何が変わるかを見せてから**適用する。
この端末のローカルLLMに直接やらせることもできる。
...
| **AIとの往復 v0.8.0（文面を渡す→直させる→口に入れると図になる。差分を見てから適用）** | **いまここ** |
...
| 手順 | 何が起きるか |
|---|---|
| ① 渡す文面をつくる | **HSLの文法 ＋ いまの構造 ＋ いま出ている指摘 ＋ してほしいこと** を1つの文にしてコピーする。これをそのままChatGPTなり手元のLLMなりに貼る |
| ② 返ってきた文を入れる | 貼ると読み取る。**``` で囲まれていても、前後に説明が付いていても、HSLでもJSONでも**取り出す |
| ③ この端末のLLMにやらせる | 選んだローカルLLMに①の文面を投げ、返答を②の欄に流し込んで自動で読み取る |

読み取ると **適用する前に差分が出る**（`+` 増える / `-` 消える / `~` 変わる）。押すまで図は変わらない。

- 検証エラーがある返答は、**適用前に**「このまま適用すると検証エラー: …」と出し、
  **そのエラーをAIに返す文面**をコピーできる。読めない返答も行番号付きで断り、同じく返す文面を出す。
- 渡す文面には文法の要点（route を書く / join には position を付ける / `.out.<field>` を見るなら schema を書く など）が
  入っているので、**LLMが初見でも書ける**ことを狙っている。
...
その結果は①の文面にも同梱されるので、AIは指摘を踏まえて直せる。
...
| **ローカルLLMにやらせる** | 返答が口に流れ込み、自動で読み取られ、適用で図になる |
