...
- **前回の続き（Checkpoint）があるときは、図もJSONも実行対象も同じものにする。**
  以前は textarea だけ前回のSpecに戻っていて、**画面は例なのに実行すると前のハーネスが動く**状態になっていた
  （実機で「例を実行できない」ように見えた原因）。お知らせの **「はじめから（例に戻す）」** で
  Checkpointを消して例に戻せる。
...
| 前回の続きがある端末 | 図・JSON・入出力欄が**すべて前回のハーネス**になる（ずれていると「画面は例なのに前のが動く」） |
| 直すところがある続き | その場で直せるボタンが出る |
| はじめから | 例に戻り、Checkpointが消え、検証エラーも無くなる |
