...
```
node tests/harness-runtime.mjs      # 自己テスト62件（Runtime回帰 + Provider契約 + HSL + 合流 + ループ + 出口 + 条件/計算 + 動く順番 + 動き出す条件）
...
node tests/harness-outputs.mjs      # 出力の項目・出口を分ける・受け取る側の項目えらび
node tests/harness-calc.mjs         # 計算ノード・条件分岐ノード・ループのJSモード・使い方の？
```
